...
def inconsistencias_cat_cols(data_A, data_B, data_C, columnas):
    inconsistencias = []

    for col in columnas:
        for i in range(len(data_A)):
            valor_A = data_A[col].iloc[i] if i < len(data_A) else None
            valor_B = data_B[col].iloc[i] if i < len(data_B) else None
            valor_C = data_C[col].iloc[i] if i < len(data_C) else None

            if valor_A != valor_B or valor_A != valor_C:
                inconsistencias.append({
                    'Indice': i,
                    'Columna': col,
                    'Fecha': data_A['timestamp'].iloc[i].date() if 'timestamp' in data_A.columns else None,
                    'Valor_A': valor_A,
                    'Valor_B': valor_B,
                    'Valor_C': valor_C
                })
...
def create_numeric_pipeline():
    ...
    """
    numeric_pipeline = Pipeline([
        ('auto_transform', FunctionTransformer(auto_data_transform, validate=False))  
    ])
